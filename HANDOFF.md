...
**라이브 = `3e2e11c` (푸시·배포 반영 완료). 로컬과 원격 동일.**
...
**`3e2e11c` 코드에 박혀 있던 회원 명단 103명 제거 (개인정보)**
- `INITIAL_SEED_MEMBERS`(103명 전원의 이름·성별·연령대·당근·출석)와 `targetAttendees`(실명 12명) 삭제.
- 시드는 "Firestore 가 완전히 빈 경우"에만 도는 경로였다. 서버에 회원이 있으므로 죽은 코드였다.
- **서버가 정말 비면 관리자 도구의 CSV 가져오기로 복구한다.** 가짜 명단을 다시 코드에 넣지 말 것
  (그 화면에서 출석 체크하면 유령 회원이 서버에 생긴다).

**`ca50118` 커플 분리를 실명 → 회원 id 로** (저장소가 public) → 아래 P3 참조
...
- **커플 분리는 회원 문서 id 로 지정한다.** `AVOID_COUPLE_PAIRS` (실명 금지 — 저장소가 public 이다).
  id 는 화면 어디에도 안 나오므로 리더가 눈으로 확인할 수단이 없다.
  그래서 `renderSeatingPlan()` 초입에서 **대상 id 가 명단에 없으면 콘솔 경고**를 한 번 남긴다.
  회원이 탈퇴 후 재가입하면 id 가 바뀌어 규칙이 죽으므로, 그 경고가 뜨면
  Firebase 콘솔 members 컬렉션에서 현재 id 를 찾아 상수를 고칠 것. 경고 로직을 지우지 말 것.
  커플이 더 늘면 회원 데이터 필드(`avoidWith`)로 뺄 것.
